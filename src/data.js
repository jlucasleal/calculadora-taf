...
export class MakeData {
	...
	getAgeRange() {
		...
		if (age >= 18 && age <= 21) {
			...
			ageRange = 13
			console.log(
				"Idade entre 40 e 45 com sustentação (LEMB Feminino). Range de idade número: 13",
			)
			// } else if (sust && age >= 46 && age <= 49) {
		} else if (sust && ageRange === 8) {
			ageRange = 14
			console.log(
				"Idade entre 46 e 49 com sustentação (LEMB Feminino). Range de idade número: 14",
			)
		} else {
	...
	getCorridaScore() {
		...
		const corrida = Number(this.corrida)
	...
	getFlexaoScore() {
		let flexaoScore = 0
		const ageRange = this.ageRange
		const flexao = Number(this.flexao)
	...
	getAbdScore() {
		let abdScore = 0
		const ageRange = this.ageRange
		const abd = Number(this.abdominal)
